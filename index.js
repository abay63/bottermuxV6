...
ator = 'SUBSCRIBE'
namo = 'ABAY'
cr = '*Subscribe:abdulaziz.albaihaqi*'//TERSERAH MAU GANTI ATAU KAGAK
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["6285694347823@s.whatsapp.net"] //GANTI NOMOR LUH
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('chanel gw','yellow'))
})
...
case 'playstore':
                ...
                ps = `${body.slice(11)}`
                  anu = await fetchJson(`http://api.lolhuman.xyz/api/playstore?apikey=LuckApikey&query=${ps}`, {method: 'get'})
                  store = '======================\n'
...
case 'memeindo':
				...
				case 'ttp': 
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/ttp?apikey=LuckApikey&text=${txt}`)
...
case 'attp':
                 if (!isRegistered) return reply( ind.noregis())
					...
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://api.vhtear.com/textxgif?text=${teks}&apikey=Beli Apikey`, {method: 'get'})
...
case 'tts':
           ...
           client.sendMessage(from, bupfer, image, {quoted: mek})
		    await limitAdd(sender)
           break 
       case 'hartatata2': // Update	
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
		...
		case 'hartatata3': // Update
				...
				bh = body.slice(12)
				reply(ind.wait())
				bh = await getBuffer(`https://api.vhtear.com/hartatahta?text=${bh}&apikey=Beli Apikey`)
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
				case 'imagetext':
                ...
                imageToBase64(`https://api.vhtear.com/textxgif?text=${args[0]}&apikey=Beli Apikey`).then((response) => {var buf = Buffer.from(response, 'base64');
...
case 'bneon':
         ...
         case 'cpubg':
					...
					reply(ind.wait())
					anu = await fetchJson(`https://ferdiz-api.herokuapp.com/api/pubg?text=${cpubg1}&text2=${cpubg2}`, {method: 'get'})
					cpubg = await getBuffer(anu.result)
					baby.sendMessage(from, cpubg, image, {quoted: mek})
					await limitAdd(sender) 
					break 
         case 'blackpink':
                case 'neon':
                ...
                case 'christmas':
                    ...
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/${command}?apikey=LuckApikey&text=${txt}`)
                   client.sendMessage(from, buffer, image, { quoted: mek })
                    break
case 'lk21':
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} Transformer`)
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/lk21?apikey=LuckApikey&query=${query}`)
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  tels = body.slice(7)
  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
  hsehe = await getBuffer(`https://videfikri.com/api/textmaker/wanted/?urlgbr=${anu.display_url}&text1=Orang Gila&text2=${tels}`)
     ...
     case 'trigger':
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
				...
				if (!isPrem) return reply(ind.premium())
                    ...
                    url = args[0]
					url =`http://api.lolhuman.xyz/api/tiktok?apikey=LuckApikey&url=https://vt.tiktok.com/ZSwWCk5o/${url}`
				...
				case 'ytmp3':
					...
					if (args.length < 1) return reply('Urlnya mana um?')
					ini_link = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytaudio?apikey=LuckApikey&url=${ini_link}`)
                    get_result = get_result.result
                    txt = `Title : ${get_result.title}\n`
                    txt += `Uploader : ${get_result.uploader}\n`
                    txt += `Duration : ${get_result.duration}\n`
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytplay?apikey=LuckApikey&query=${query}`)
                    get_result = get_result.result
		...
		if (!isRegistered) return reply(ind.noregis())
			if (args.length < 1) return reply(`Contoh: ${prefix}joox Kokoronashi`)
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/jooxplay?apikey=LuckApikey&query=${query}`)
                    get_result = get_result.result
		    get_info = get_result.info
		    txt = '*_[ FITUR UP BY ABAY ]_*\n\n'
                    ...
                    case 'fbdl':
                    ini_url = args[0]
                    ini_url = await fetchJson(`http://api.lolhuman.xyz/api/facebook?apikey=LuckApikey&url=${ini_url}`)
...
║│【✿͜͡➪ *XP : ${getLevelingXp(sender)}/${reqXp} *
║│【✿͜͡➪ *Level : ${getLevelingLevel(sender)}*
║│【✿͜͡➪ *Role : ${role}*
║│【✿͜͡➪ *User register : ${_registered.length}*
║
║│【✿͜͡➪ 「 *ABOUT BOT* 」
│╭───────────────────
▬▭▬▭▬❰ *╭───────────────────
...
client.sendMessage(from, menu2, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
				case 'info':
					me = client.user
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻,𝗼𝘁𝘄 𝗸??𝗰𝗸 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
