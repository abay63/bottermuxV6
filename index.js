...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('MrG{108P}','yellow'))
})
...
case 'tts':
           ...
           client.sendMessage(from, bupfer, image, {quoted: mek})
		    await limitAdd(sender)
           break 
       case 'hartatata2': // Update MrG{108P}*		
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
